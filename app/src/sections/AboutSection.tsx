...
const stats = [
  { value: '3', label: 'Research Groups', color: '#7C3AED', bg: 'rgba(124, 58, 237, 0.1)' },
  { value: '7', label: 'Faculty Members', color: '#14B8A6', bg: 'rgba(20, 184, 166, 0.1)' },
  { value: '100+', label: 'IEEE Trans. Papers', color: '#0EA5E9', bg: 'rgba(14, 165, 233, 0.1)' },
  { value: '10+', label: 'Provincial Awards', color: '#F59E0B', bg: 'rgba(245, 158, 11, 0.1)' },
];
    ...
    <section id="about" className="relative py-16 lg:py-24" style={{ background: '#0A0A12' }}>
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        >
          ...
          <h2 className="display-lg text-[#F0F0F5]">About AGI&FBHC</h2>
        </motion.div>
        ...
        <div className="grid grid-cols-1 gap-12 lg:grid-cols-2">
          ...
          >
            ...
            >
              本团队主要聚焦于通用人工智能行业应用，以及与食品生物健康等学科的交叉创新理论研究。
              曾获教育部科技进步一等奖等奖励，牵头承担国家重点研发计划等国家级项目，
              发表权威期刊 IEEE Trans. 系列论文 100 余篇，培育江苏省优秀博士/硕士论文 10 余次。
            </p>

            {/* Faculty Members */}
            <div className="mt-6">
              <h4 className="mb-2.5 text-sm font-medium text-[#F0F0F5]">团队成员</h4>
              <div className="flex flex-wrap gap-2">
                {facultyMembers.map((member) => (
                  <span
                    ...
                    style={{
                      background: member.role === '负责人'
                        ? 'rgba(124, 58, 237, 0.12)'
                        : 'rgba(255, 255, 255, 0.05)',
                      border: `1px solid ${member.role === '负责人'
                        ? 'rgba(124, 58, 237, 0.25)'
                        : 'rgba(255, 255, 255, 0.08)'}`,
                      color: member.role === '负责人' ? '#A78BFA' : '#8B8B9E',
                    }}
                  >
                    {member.name}
                    {member.role === '负责人' && (
                      <span className="text-[10px] opacity-70">负责人</span>
                    )}
                  </span>
                ))}
              </div>
            ...
            {/* Honors */}
            <div className="mt-5">
              <h4 className="mb-2.5 text-sm font-medium text-[#F0F0F5]">人才与荣誉</h4>
              <div className="flex flex-wrap gap-2">
                {honors.map((honor) => (
                  <span
                    key={honor}
